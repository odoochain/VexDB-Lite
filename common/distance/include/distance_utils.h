...
#if ANN_GCC_VERSION_GE(8, 0)
#define INLINE_PROP FORCE_INLINE
#else
#define INLINE_PROP
#endif

#define USE_SIMPLE_TEMPLATE ANN_COMPILER_GCC && !ANN_GCC_VERSION_GE(8, 0)
#if USE_SIMPLE_TEMPLATE
#include "distance/distance_template_simple.h"
#include "distance/transform_template_simple.h"
#else
#include "distance/include/distance_template.h"
#include "distance/include/transform_template.h"
#endif
...
#define GENERAL_FUNC(name) general_##name
...
struct DistanceDispatcher {
    ...
    static void get_distance_batch2(const void *x, void *const *y, uint16 dim, uint16 y_size, float *out)
    {
        ...
        for (uint16 i = 0; i < y_size; ++i) {
            ann_helper::unroll<helper::k_bytes * helper::skip_step / helper::cache_size>(
                [&](auto j) -> void {
                    favored_prefetch((char *)yp[i] + j * helper::cache_size);
                });
