...
class GraphIndexAlgorithm : public PERFER(AlgPerfCats) {
    ...
    using UMap = UnorderedMap<K, V, H, E, Alloc<Pair<K, V>>>;
    ...
    Store &store;
    Dister &distancer;
    Holder<UMap<Pair<T, T>, float, PairHasher, PairCmp>> dist_cache;
public:
...
retry:
        ...
        Vec<Cand> nbr_record[search_level]; /* record nbr in search */
        ...
        switch (strat) {
            ...
            case InsertStrategy::InsertPoint: {
                ...
                Span<Vec<Cand>> nbr_span{nbr_record, (size_t)search_level};
    ...
    void repair_upperpoint(T cur_layer_idx, const UnorderedSet<size_t> &deleted)
    {
        ...
        T new_neighbors_info[nbr_num * 2]; /* nbr_num = m, max 256 total */
        ...
        store.set_upper_neighbors(cur_layer_idx, new_neighbors_info);
...
private:
    ...
    void init_range_elem(PointExtensionContext &ctx, T id, Span<TidHolder> data)
    {
        ...
        Span<const typename point_type::Data> tids(temp, data.size());
                           ...
                           const char *query)
    {
        ...
        closest.emplace(store.get_data(new_point_id), (T)INVALID_VECTOR_ID, self.dist, new_point_id);
        ...
        do {
            ...
            CONSTEXPR_IF (use_dist_cache) {
                ...
                } else if (cur_point.id == new_point_id) {
                    cur_point_is_closer = elem_closer(r, cur_point);
                    if (cur_point_is_closer) {
                        added->push_back(cur_point);
                    }
...
#pragma GCC diagnostic pop
    ...
    void add_upperpoint(T cur_layer_idx, T lower_layer_idx, T id, const Vec<Cand> &neighbors)
    {
        ...
        T neighbors_info[nbr_num * 2]; /* nbr_num = m, max 256 total */
    ...
    void add_basepoint(T id, const Vec<Cand> &neighbors)
    {
        ...
        T neighbors_id[nbr_num]; /* nbr_num = m*2, max 256 */
        for (uint_fast16_t i = 0; i < nbr_num; ++i) {
    ...
    void add_first_basepoint()
    {
        uint_fast16_t nbr_num = m * 2;
        T neighbors_id[nbr_num]; /* nbr_num = m*2, max 256 */
        for (uint_fast16_t i = 0; i < nbr_num; ++i) neighbors_id[i] = (T)INVALID_VECTOR_ID;
        store.add_basepoint(0, neighbors_id);
    }

    void add_first_upperpoint(T cur_idx, T lower_layer_idx, T id)
    {
        T neighbors_info[m * 2]; /* m*2, max 256 */
        for (uint_fast16_t i = 0; i < m * 2; ++i) neighbors_info[i] = (T)INVALID_VECTOR_ID;
    ...
    void set_base_neighbors(T id, const Vec<Cand> &neighbors)
    {
        ...
        store.set_base_neighbors(id, neighbors_id);
    }
