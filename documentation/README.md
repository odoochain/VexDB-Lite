...
VexDB 是一款高性能向量检索插件，提供 PostgreSQL（`vexdb_vector`）和 DuckDB（`vex`）两种适配形式，共享同一套 HNSW 图索引算法内核。
...
2. 下载 Release 中的 `vexdb_vector.so` 并参考 [功能文档](features.md#postgresql-插件) 建表

### 我想用 DuckDB 扩展
1. 查看 [兼容性表](compatibility.md) 确认 DuckDB 版本
2. 下载 `vex.duckdb_extension` 并参考 [功能文档](features.md#duckdb-扩展)
